...
import { initRedis, attachRedisTo, getRedisClient, redisAvailable } from './infra/redis.js';
...
export async function runShard() {
  ...
  try { process.title = `void-bot:shard:${cfg.shardId}/${cfg.shardCount}`; } catch {}
  ...
  Void.shardInfo = { id: cfg.shardId, count: cfg.shardCount, managed: cfg.managerMode };
  ...
  Void.healthDetails = async () => {
    ...
    try { await initDb(); db = true; } catch {}
    try { const pong = await Void.redis.ping(); redis = !!pong; } catch {}
...
function setupReadyHandler(client, cfg, ipc, router) {
  client.once(Events.ClientReady, async (readyClient) => {
    log.info({ id: readyClient.user.id, tag: readyClient.user.tag, shardId: cfg.shardId }, 'Shard client is ready');
    ...
    try {
      ...
      if (text) {
          ...
          await client.user.setPresence({
            status: 'online',
            activities: [{ name: `Shard ${cfg.shardId + 1}/${cfg.shardCount}`, type: ActivityType.Watching }],
          });
      }
    } catch (e) {
    ...
    try {
      ...
      log.info({ count: guilds.length, shardId: cfg.shardId }, 'Ensured guild_config rows');
    } catch (err) {
...
function setupShardEvents(client, cfg, ipc) {
  client.on(Events.GuildCreate, g => {
    log.info({ guildId: g.id, guildName: g.name, shard: cfg.shardId }, 'Joined guild');
    ipc?.send('guildUpdate', { action: 'join', guild: { id: g.id, name: g.name } });
  });
  client.on(Events.GuildDelete, g => {
    log.info({ guildId: g.id, guildName: g.name, shard: cfg.shardId }, 'Left guild');
    ipc?.send('guildUpdate', { action: 'leave', guild: { id: g.id, name: g.name } });
  });
  client.on(Events.ShardError, (err, shardId) => log.error({ err, shardId }, 'Shard WS error'));
  client.on(Events.ShardResume, (shardId) => log.info({ shardId }, 'Shard resumed'));
  client.on(Events.ShardDisconnect, (_, shardId) => log.warn({ shardId }, 'Shard disconnected'));
}
...
function startShardJobWorker(client, router) {
  ...
  if (client.shard.id === 0) {
    const cleanupInterval = setInterval(async () => {
      try { const del = await cleanupOldJobs(30); if (del > 0) log.info({ deleted: del }, 'Cleaned old jobs'); }
      catch (err) { log.error({ err }, 'Cleanup error'); }
    }, 86_400_000); // 24 hours
  ...
  if (process.send) {
    process.on('message', msg => { if (msg?.type === 'jobNudge') processJobs(); });
  }
...
function setupShutdownHandlers(client, ipc) {
  ...
  const shutdown = async (signal) => {
    ...
    try { ipc?.send('shutdown', { shard: client.shard.id }); } catch {}
    try { await client.destroy(); } catch (err) { log.error({ err }, 'Destroy error'); }
    process.exit(0);
  };
  process.once('SIGINT', () => shutdown('SIGINT'));
  process.once('SIGTERM', () => shutdown('SIGTERM'));
  if (process.send) process.on('message', msg => { if (msg?.type === 'shutdown') shutdown('MANAGER'); });
  process.on('uncaughtException', err => { log.fatal({ err, shard: client.shard.id }, 'Uncaught'); shutdown('UNCAUGHT_EXCEPTION'); });
  process.on('unhandledRejection', reason => { log.error({ reason, shard: client.shard.id }, 'Unhandled rejection'); });
}

runShard().catch(err => { log.fatal({ err }, 'Shard start failed'); process.exit(1); });