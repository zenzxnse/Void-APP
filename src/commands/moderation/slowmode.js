...
export default {
  data: new SlashCommandBuilder()
    ...
    .addStringOption(o => o
      .setName('delay')
      .setDescription('Slowmode delay (e.g., 5s, 10s, 1m, 5m) or "off" to disable')
      .setRequired(true)
      .setMaxLength(20))
  ...
  async execute(interaction) {
    ...
    const delayStr = interaction.options.getString('delay', true).trim().toLowerCase();
    ...
    if (channel.type === ChannelType.GuildAnnouncement || channel.type === ChannelType.GuildStageVoice) {
      return safeReply(interaction, {
    ...
    if (['off', '0', 'none', 'disable', 'remove'].includes(delayStr)) {
      ...
      if (!slowmodeSeconds || slowmodeSeconds < 0) {
        return safeReply(interaction, {
          content: '❌ Invalid delay format. Examples: `5s`, `10s`, `1m`, `5m` or `off`',
          flags: MessageFlags.Ephemeral
        });
      }
    ...
    try {
      ...
      await channel.setRateLimitPerUser(slowmodeSeconds, `[${interaction.user.tag}] ${reason.substring(0, 400)}`);
        ...
        if (tempDurationSeconds && slowmodeSeconds > 0) {
          ...
          const { rows: [job] } = await client.query(
            `INSERT INTO scheduled_jobs (type, guild_id, channel_id, run_at, priority, data)
             VALUES ($1, $2, $3, $4, $5, $6)
             RETURNING id`,
            ['slowmode_end', interaction.guildId, channel.id, removeAt, 40, JSON.stringify({
      ...
      const response = tempDurationSeconds
        ? [
            `✅ **Set slowmode in ${channel}**`,
            `⏱️ **Delay:** ${prettySecs(slowmodeSeconds)}`,
            `⏰ **Duration:** ${prettySecs(tempDurationSeconds)}`,
            `📅 **Removes:** <t:${Math.floor((Date.now() + tempDurationSeconds * 1000) / 1000)}:F> (<t:${Math.floor((Date.now() + tempDurationSeconds * 1000) / 1000)}:R>)`,
            `📝 **Reason:** ${reason}`
          ].join('\n')
        : [
            `✅ **Set slowmode in ${channel}**`,
