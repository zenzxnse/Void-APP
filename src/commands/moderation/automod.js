...
function parseActions(primaryAction, actionsCsv) {
  ...
  if (actionsCsv) {
    for (const raw of actionsCsv.split(',').map(s => s.trim()).filter(Boolean)) {
  ...
  // validate
  const invalid = normalized.filter(a => !ACTIONS.includes(a) && a !== 'timeout'); // 'mute' already mapped
  if (invalid.length) {
    return { ok: false, error: `Invalid action(s): ${invalid.join(', ')}`, actions: [] };
  }

  // remove duplicates, preserve a deterministic order by ACTIONS array
  const ordered = ACTIONS.filter(a => normalized.includes(a)).map(a => (a === 'mute' ? 'timeout' : a));
  // ensure at least one
  if (ordered.length === 0) return { ok: false, error: 'No valid actions provided', actions: [] };
  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(sub => sub
      ...
      .addStringOption(o => o
        ...
        .addChoices(...Object.entries(RULE_TYPES).map(([key, rule]) => ({
          name: rule.name,
          value: key
        })))
      )
    ...
    .addSubcommand(sub => sub
      ...
      .addStringOption(o => o
        .setName('actions')
        .setDescription('Comma-separated actions to add (e.g., delete,warn,timeout)')
        .setRequired(true)
        .setMaxLength(100)
      )
      .addStringOption(o => o
        .setName('rule_key')
        .setDescription('Rule key (e.g., spam_5). If omitted, use type+threshold')
        .setMaxLength(50)
      )
      .addStringOption(o => o
        .setName('type')
      ...
      .addIntegerOption(o => o
        .setName('threshold')
        .setDescription('Threshold used in rule_key (if no rule_key provided)')
        .setMinValue(1)
        .setMaxValue(100)
      )
      .addStringOption(o => o
    ...
    .addSubcommand(sub => sub
      ...
      .addStringOption(o => o
        .setName('actions')
        .setDescription('Comma-separated actions to remove (e.g., delete,warn,timeout)')
        .setRequired(true)
        .setMaxLength(100)
      )
      .addStringOption(o => o
    ...
    .addSubcommand(sub => sub
      ...
      .addStringOption(o => o
        .setName('actions')
        .setDescription('Comma-separated actions (e.g., delete,warn,timeout)')
        .setMaxLength(100)
      )
      .addIntegerOption(o => o
        .setName('threshold')
...
async function handleDelAction(interaction, autoModState) {
  ...
  if (!parsed.ok) {
    return safeReply(interaction, { content: parsed.error, flags: MessageFlags.Ephemeral });
  }
  const toRemove = new Set(parsed.actions.map(a => (a === 'mute' ? 'timeout' : a)));
  ...
  if (!ruleKey) {
    if (typeOpt && (threshOpt != null)) {
      ...
      return safeReply(interaction, {
        content: 'Provide either `rule_key`, or `type` (+ optional `threshold`).',
        flags: MessageFlags.Ephemeral
      });
    }
  }
  ...
  const current = Array.isArray(data.actions) && data.actions.length
    ? data.actions
    : (rule.action ? [rule.action] : []);

  // compute new action set
  const after = ACTIONS
    .filter(a => current.map(x => (x === 'mute' ? 'timeout' : x)).includes(a))
    .filter(a => !toRemove.has(a));

  if (after.length === 0) {
    return safeReply(interaction, {
      content: 'Refused: a rule must keep at least one action. Remove fewer actions or add new ones first.',
      flags: MessageFlags.Ephemeral
    });
  }
  ...
  try {
    ...
    return safeReply(interaction, { content: 'Failed to update rule actions.', flags: MessageFlags.Ephemeral });
  }
}
...
async function handleAddAction(interaction, autoModState) {
  ...
  const existingActions = Array.isArray(existingData.actions) && existingData.actions.length
    ? existingData.actions
    : (rule.action ? [rule.action] : []);

  // Merge, normalize & order by canonical ACTIONS order
  const mergedSet = new Set([...existingActions, ...actionsToAdd].map(a => (a === 'mute' ? 'timeout' : a)));
  const orderedMerged = ACTIONS.filter(a => mergedSet.has(a)).map(a => (a === 'mute' ? 'timeout' : a));
  ...
  if (includesTimeout) {
    if (durationStr) {
      ...
      newTimeoutSeconds = rule.duration_seconds ?? DEFAULT_ACTION_DURATION_SECONDS;
    }
  }
  ...
  try {
      ...
      await client.query(
        ...
        [
          ...
          JSON.stringify({
            ...
            newTimeoutSeconds: includesTimeout ? (newTimeoutSeconds ?? null) : null
          })
        ]
      );
    ...
    bits.push(`**Actions (now):** ${orderedMerged.map(a => a.toUpperCase()).join(' + ')}`);
    if (includesTimeout) {
      const t = newTimeoutSeconds ?? updated.duration_seconds;
      bits.push(`**Timeout Duration:** ${prettySecs(t)}`);
    }

    return safeReply(interaction, { content: bits.join('\n'), flags: MessageFlags.Ephemeral });
  ...
  if (!already) {
    await tx(async (client) => {
      ...
      await client.query(
        `INSERT INTO audit_logs (guild_id, action_type, actor_id, target_id, details)
         VALUES ($1, $2, $3, NULL, $4)`,
        [interaction.guildId, 'automod_enable', interaction.user.id, JSON.stringify({ timestamp: Date.now() })]
      );
    });
    await autoModState.invalidateGuildConfig(interaction.guildId);
  }
...
async function handleDisable(interaction, autoModState) {
  ...
  const result = await tx(async (client) => {
    if (!ruleType) {
      ...
      await client.query(
        `INSERT INTO audit_logs (guild_id, action_type, actor_id, target_id, details)
         VALUES ($1, $2, $3, NULL, $4)`,
        [interaction.guildId, 'automod_disable_all', interaction.user.id, JSON.stringify({ rulesDisabled: rowCount })]
      );
      ...
      if (rowCount > 0) {
        await client.query(
          `INSERT INTO audit_logs (guild_id, action_type, actor_id, target_id, details)
           VALUES ($1, $2, $3, NULL, $4)`,
          [interaction.guildId, 'automod_disable_rule', interaction.user.id, JSON.stringify({ ruleType, rulesDisabled: rowCount })]
        );
      }
  ...
  if (result.type === 'all') {
    return safeReply(interaction, {
      content: `Auto-moderation system disabled${result.count > 0 ? `\nDisabled ${result.count} rule${result.count !== 1 ? 's' : ''}` : ''}`,
      flags: MessageFlags.Ephemeral
    });
  } else {
    if (result.count === 0) {
      return safeReply(interaction, {
        content: `No ${RULE_TYPES[result.ruleType]?.name || result.ruleType} rules found to disable.`,
        flags: MessageFlags.Ephemeral
      });
    }
    return safeReply(interaction, {
      content: `Disabled ${result.count} ${RULE_TYPES[result.ruleType]?.name || result.ruleType} rule${result.count !== 1 ? 's' : ''}`,
      flags: MessageFlags.Ephemeral
    });
  }
}
...
async function handleList(interaction) {
  ...
  const [systemResult, rulesResult] = await Promise.all([
    query(`SELECT auto_mod_enabled FROM guild_config WHERE guild_id = $1`, [interaction.guildId]),
    query(`
  ...
  embed.addFields({ name: 'System Status', value: systemEnabled ? 'Enabled' : 'Disabled', inline: true });

  if (rules.length === 0) {
    embed.setDescription('No auto-moderation rules configured.\n\nUse `/automod rule` to create your first rule.');
  ...
  embed.addFields({
    name: 'Rules Summary',
    value: `${activeRules}/${rules.length} active rules${quarantined > 0 ? `\n${quarantined} quarantined` : ''}`,
    inline: true
  });

  // derive actions list (rule_data.actions or single action)
  const displayActions = (rule) => {
    const d = rule.rule_data && (typeof rule.rule_data === 'object' ? rule.rule_data : safeJson(rule.rule_data));
    const acts = Array.isArray(d?.actions) && d.actions.length ? d.actions : (rule.action ? [rule.action] : []);
  ...
  if (detailed) {
    ...
    for (const [type, typeRules] of Object.entries(grouped)) {
      ...
      const lines = typeRules.map(rule => {
        const status = rule.quarantined ? 'QUARANTINED' : (rule.enabled ? 'ACTIVE' : 'DISABLED');
        const unit = RULE_TYPES[type]?.thresholdUnit || '';
        const tStr = rule.threshold ? ` (${rule.threshold}${unit ? ' ' + unit : ''})` : '';
        const acts = displayActions(rule);
        return `**${rule.name}**: ${status} → ${acts}${tStr} [v${rule.version ?? 1}]`;
      });
    ...
    const summary = Object.entries(
      ...
      return `**${ruleInfo?.name || type}**: ${stats.active}/${stats.total} active${stats.quarantined > 0 ? ` (${stats.quarantined} quarantined)` : ''}`;
    });

    embed.addFields({ name: 'Rule Types', value: summary.join('\n'), inline: false });
  }
...
async function handleRule(interaction, autoModState) {
  ...
  const ruleInfo = RULE_TYPES[type];
  if (!ruleInfo) {
    return safeReply(interaction, { content: 'Invalid rule type.', flags: MessageFlags.Ephemeral });
  }

  // Parse + validate actions (allow multiple). Map mute → timeout.
  const parsedActions = parseActions(primaryAction, actionsCsv);
  if (!parsedActions.ok) {
    return safeReply(interaction, { content: parsedActions.error, flags: MessageFlags.Ephemeral });
  }
  ...
  if (ruleInfo.requiresThreshold) {
    ...
    if (threshold > (ruleInfo.maxThreshold ?? Number.MAX_SAFE_INTEGER)) {
      errors.push(`Threshold cannot exceed ${ruleInfo.maxThreshold} ${ruleInfo.thresholdUnit || ''} for ${ruleInfo.name}`);
    }
    if (threshold < 1) errors.push(`Threshold must be at least 1 for ${ruleInfo.name}`);
  }
  ...
  if (windowStr) {
    ...
    if (!parsed || parsed <= 0) errors.push('Invalid window format. Examples: 5s, 1m');
    else if (ruleInfo.maxWindowSeconds && parsed > ruleInfo.maxWindowSeconds) {
      errors.push(`Window cannot exceed ${prettySecs(ruleInfo.maxWindowSeconds)}`);
    } else {
  ...
  if (actions.includes('timeout')) {
    if (durationStr) {
      ...
      if (!parsed || parsed <= 0) errors.push('Invalid duration format. Examples: 5m, 1h, 1d');
  ...
  // Pattern validation
  if (ruleInfo.requiresPattern && !pattern) errors.push(`${ruleInfo.name} requires a pattern`);
  if (pattern && type === 'regex') {
    if (pattern.length > (ruleInfo.maxPatternLength || 200)) {
      errors.push(`Regex pattern too long (max ${ruleInfo.maxPatternLength || 200} characters)`);
    }
    try { new RegExp(pattern, 'giu'); } catch (err) { errors.push(`Invalid regex pattern: ${err.message}`); }
  }

  // Exemptions
  const exemptRoles = validateRoleIds(exemptRolesStr);
  const exemptChannels = validateChannelIds(exemptChannelsStr);
  if (exemptRolesStr && exemptRoles.length === 0) errors.push('No valid role IDs found in exempt_roles');
  if (exemptChannelsStr && exemptChannels.length === 0) errors.push('No valid channel IDs found in exempt_channels');
  ...
  const ruleName = `${ruleInfo.name}${threshold ? ` (${threshold}${ruleInfo.thresholdUnit ? ' ' + ruleInfo.thresholdUnit : ''})` : ''}`;
  ...
  try {
    ...
    bits.push(`**Actions:** ${actions.map(a => a.toUpperCase()).join(' + ')}${actions.includes('timeout') && primaryAction === 'mute' ? ' (mute → TIMEOUT)' : ''}`);
    if (threshold) bits.push(`**Threshold:** ${threshold} ${RULE_TYPES[type]?.thresholdUnit || ''}`);
    if (ruleData.window_seconds) bits.push(`**Detection Window:** ${prettySecs(ruleData.window_seconds)}`);
    if (actionDurationSeconds && actions.includes('timeout')) bits.push(`**Timeout Duration:** ${prettySecs(actionDurationSeconds)}`);
    if (pattern) bits.push(`**Pattern:** \`${pattern.substring(0, 50)}${pattern.length > 50 ? '...' : ''}\``);
    if (exemptRoles.length) bits.push(`**Exempt Roles:** ${exemptRoles.length}`);
    if (exemptChannels.length) bits.push(`**Exempt Channels:** ${exemptChannels.length}`);
...
async function handleTest(interaction) {
  ...
  return safeReply(interaction, {
    content: `Test functionality not yet implemented.\n\nRule: \`${ruleKey}\`\nText: \`${testText.substring(0, 100)}${testText.length > 100 ? '...' : ''}\``,
    flags: MessageFlags.Ephemeral
  });
}
